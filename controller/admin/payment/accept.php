<?php
    ...
    if($_SESSION['status']!="admin"){
        ...
        $stmt = $mysqli->prepare("update contract set overdue = ? where id = ?");
        $stmt->bind_param("ii",$overdue,$contractid);
        $result = $stmt->execute();
        if($result){
            $stmt = $mysqli->prepare("update payment set status = 'accepted' where id = ?");
            ...
            if($result){
                ...
                $message = "Your Payment for \nContractID: {$contractid}\nRoom: {$roomname}\nAmount: {$amount}\nHas been accepted, you current overdue is RM {$overdue}";   
                $headers = "From:" . $from . "\r\n";
