              ...
              <a href="index.html">           
                  ...
                  <div class="card card-table-border-none" id="recent-orders">
                    <div class="card-header justify-content-between">
                      <h2>Recent Payment Submission</h2>
                    </div>
                    <div class="card-body pt-0 pb-5">
                      <table class="table card-table table-hover table-responsive table-responsive-large" style="width:100%">
                        ...
                        <tbody >
                              <?php
                                    ...
                                    $total_pages_sql = $mysqli->query("SELECT COUNT(*) FROM payment");
                                    $total_rows = mysqli_fetch_array($total_pages_sql)[0];
                                    $total_pages = ceil($total_rows / $no_of_records_per_page);

                                    $check = $mysqli->prepare("select payment.amount,student.id,payment.id,contract.id, student.firstName, student.lastName, room.name, contract.overdue, payment.status, payment.file from payment join contract
                                    on contract.id = payment.contract join student ON student.id = contract.student join room ON room.id = contract.room order by  (select FIELD(status, 'waiting', 'rejected', 'accepted') ) LIMIT $offset, $no_of_records_per_page");
                                    
                                    $check->execute();
                                    $check->store_result();
                                    $check->bind_result($amount,$studentid,$id,$contractid,$firstName,$lastName,$roomname,$overdue, $status, $file);

                                    if($check->num_rows > 0){
                                      while($row = $check->fetch()){
                              ?>
                          <tr>
                            <!-- <td ><?php echo " ".htmlspecialchars($contractid) ; ?></td> -->
                            <td >
                              <a class="text-dark" href="../../../admin/contract/edit/?id=<?php echo $contractid;?>"> <?php echo htmlspecialchars($contractid) ; ?></a>
                            </td>
                            <td >
                            ...
                            <td >
                              <a href="../../../uploads/payment/<?php echo $file?>" class="text-dark"> <?php echo htmlspecialchars($file) ; ?></a>
                            </td>
                            <td class="d-none d-md-table-cell"><?php echo htmlspecialchars($amount) ; ?></td>
                            <td >
                            <?php
                      ...
                      <?php if(isset($_GET['message'])) {
                              ...
                              <?php
                                      } else if($_GET['message'] == "successaccept"){
                                        ?>
                            <div class="col-12" style="margin-top: 40px">
                                <div class="alert alert-success alert-dismissible fade show"
                                    style="margin-bottom: 30px">
                                    <strong>Success!</strong> You have accepted the booking request!
                                    <button type="button" class="close" data-dismiss="alert">&times;</button>
                                </div>
                            </div>
                            <?php
                          ...
                          <?php
                                ...
                                <div class="col-12" style="margin-top: 40px">
                                    <div class="alert alert-success alert-dismissible fade show"
                                        style="margin-bottom: 30px">
                                        <strong>Success!</strong> Your have rejected the booking request!
                                        <button type="button" class="close" data-dismiss="alert">&times;</button>
                                    </div>
                                </div>
                                <?php
